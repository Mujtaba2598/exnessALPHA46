...
console.log('🕋 HALAL EXNESS BOT - FINAL WORKING');
...
class DirectTradingEngine {
    ...
    async start() {
        console.log(`🕋 Starting DIRECT TRADING for ${this.userEmail}`);
    ...
    async tryTrade() {
        try {
            ...
            console.log(`🔥 ATTEMPTING TRADE: ${symbol}`);
            ...
            const entryPrice = price.ask || price.bid || 0;
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        const engine = new DirectTradingEngine(sessionId, req.user.email, config, user.tickerallSessionId);
        ...
        res.json({
            success: true,
            sessionId,
            message: `🕋 DIRECT TRADING STARTED! 🔥 First trade opening NOW! Investment: $${investmentAmount} | Target: $${targetProfit}`
        });
    } catch (error) {
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🕋 HALAL EXNESS BOT - FINAL WORKING`);
